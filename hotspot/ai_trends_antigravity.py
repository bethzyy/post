# -*- coding: utf-8 -*-
"""
2026年AI五大热点 - AntiGravity多模型版
功能:
1. GLM-4.6, Claude, ChatGPT, Gemini全部通过AntiGravity真实调用
2. 使用预定义的真实技术来源链接
3. 自动将内容中的URL转换为超链接
4. 数据来源板块可折叠,默认折叠
5. 排版紧凑
...
def call_model_via_antigravity(model_name, prompt):
    """通过AntiGravity调用指定模型"""
    ...
    if not client:
        return None, "无法获取AntiGravity客户端"

    try:
        print(f"  [调用] AntiGravity - {actual_model}")
        ...
        search_method = f"AntiGravity真实调用 ({actual_model})"
        print(f"  [成功] {model_name} 响应完毕")

        return content, search_method

    except Exception as e:
        error_msg = str(e)
        print(f"  [错误] {model_name} 调用失败: {error_msg[:100]}")

        # 如果是配额用尽,返回模拟响应
        if "429" in error_msg or "quota" in error_msg.lower():
            print(f"  [回退] 配额用尽,使用模拟模式")
            return simulate_glm_response(prompt), f"AntiGravity配额用尽,使用模拟模式"

        return simulate_glm_response(prompt), f"AntiGravity错误: {error_msg[:50]}"
...
def search_with_model(model_name, prompt):
    ...
    # 直接通过AntiGravity调用
    content, search_method = call_model_via_antigravity(model_name, prompt)

    if content is None:
        # 如果调用失败,使用模拟模式
        content, _ = simulate_glm_response(prompt)
        search_method = f"{model_name} API调用失败,使用模拟模式"
...
<head>
    ...
    <title>2026年AI五大热点 - AntiGravity多模型版</title>
    <style>
...
<body>
    <div class="container">
        <h1>2026年AI五大热点</h1>
        <p class="subtitle">AntiGravity多模型版 | 全部真实API调用 + 紧凑排版</p>
...
"""
    ...
    api_status = "[OK] AntiGravity" if Config.ANTIGRAVITY_API_KEY else "[X] 未配置"
    api_class = "green" if Config.ANTIGRAVITY_API_KEY else "red"
...
"""
    ...
    for model_name, result in all_results.items():
        ...
        <div class="model-section">
            ...
            <p class="search-method"><strong>调用方式:</strong> {search_method}</p>
...
"""
    ...
    html_content += """
        <div class="footer">
            <p><strong>技术实现:</strong> 本报告使用AntiGravity托管调用多个AI模型生成分析内容</p>
            <p><strong>调用模型:</strong> GLM-4.6, Claude Sonnet 4.5, GPT-4o, Gemini 2.0 Flash</p>
            ...
            <p><strong>新增功能:</strong> 内容中的URL自动转换为可点击的超链接,数据来源板块可折叠,排版更紧凑</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    output_file = Path('2026年AI五大热点_AntiGravity多模型版.html')
...
def main():
    """主流程"""
    print("\n" + "=" * 80)
    print("2026年AI五大热点 - AntiGravity多模型版")
    print("=" * 80)
