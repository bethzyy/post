...
"""
    ...
    for model_name, result in all_results.items():
        ...
        def format_titles(text):
            def replace_title(m):
                title = m.group(1)
                return f'<div class="hotspot-title-box" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 14px 24px; border-radius: 10px; margin: 20px 0 16px 0; box-shadow: 0 4px 12px rgba(102, 126, 234, 0.25);"><div style="color: white; font-size: 1.4em; font-weight: 700; letter-spacing: 0.5px;">{title}</div></div>'
            text = re.sub(r'【([^】]+)】', replace_title, text)
            # 减少空行
            text = re.sub(r'
{3,}', '
...
', text)
        ...
        content_formatted = format_titles(content_with_links)
