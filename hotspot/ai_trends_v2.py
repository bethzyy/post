# -*- coding: utf-8 -*-
"""
2026年AI五大热点 - 多模型真实调用版 V2
功能:
1. 通过AntiGravity调用GLM-4.6、Claude、ChatGPT、Gemini真实API
2. 保留URL自动转超链接功能
3. 保留数据来源可折叠功能
4. 每个模型独立分析2026年AI热点
5. 改进:使用最新的Gemini模型名称
6. 改进:更好的错误处理,不在遇到429时立即降级
"""
...
<head>
    ...
    <title>2026年AI五大热点 - 多模型真实调用版 V2</title>
    <style>
...
<body>
    <div class="container">
        <h1>2026年AI五大热点</h1>
        <p class="subtitle">多模型真实调用版 V2 | 使用最新Gemini模型名称</p>

        <div class="info-bar">
            <span class="info-tag blue">调用方式: AntiGravity托管</span>
            <span class="info-tag purple">参与模型: 5个</span>
...
"""
    ...
    for model_name, result in all_results.items():
        badge = "对话模型" if "GPT" in model_name or "GLM" in model_name else "多模态"
...
"""
    ...
    html_content += """
        <div class="footer">
            ...
            <p><strong>改进点:</strong> 使用最新的Gemini 2.5模型,优化错误处理策略</p>
        </div>
    </div>
</body>
</html>
"""
    ...
    output_file = Path('2026年AI五大热点_多模型V2.html')
...
def main():
    ...
    models_config = {
        ...
        "ChatGPT": "gpt-4o",
        "Gemini 3 Pro": "gemini-2.5-pro-preview",
    ...
    for model_name, model_id in models_config.items():
        ...
        try:
            ...
            print(f"[失败] {model_name} 分析失败: {error_type}")
            print(f"[详情] {e}")
            print(f"[跳过] 继续下一个模型...")
            continue
    ...
    if not all_results:
        print("\n[错误] 所有模型调用都失败了")
        print("建议:")
        print("  1. 检查AntiGravity服务是否正常运行")
        print("  2. 检查API配额是否充足")
        print("  3. 尝试使用ai_trends_final.py(使用GLM-4.6官方API)")
        return
    ...
    print(f"\n新功能:")
    print(f"  [OK] 通过AntiGravity真实调用多个大模型")
    print(f"  [OK] 使用最新的Gemini 2.5 Pro和Flash模型")
    print(f"  [OK] 内容中的URL自动转换为可点击的超链接")
    print(f"  [OK] 数据来源板块可折叠,默认折叠状态")
    print(f"  [OK] 优化的错误处理策略")
