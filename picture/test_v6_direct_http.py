# -*- coding: utf-8 -*-
"""
测试V6版本的图生图功能(直接HTTP API)
验证image_urls参数通过直接HTTP调用是否工作
"""
...
def test_image_to_image():
    """测试图生图功能"""
    print("\n" + "="*80)
    print("测试 V6 图生图功能 (直接HTTP API调用)")
    print("="*80)
    ...
    print(f"  模式: 参考图片")
    print(f"  风格: 国风水墨")
    print(f"  关键改进: 直接HTTP调用Volcano API")
...
def main():
    ...
    print("  1. 验证V6版本已启动")
    print("  2. 测试参考图片上传")
    print("  3. 验证直接HTTP API调用(绕过OpenAI SDK)")
    print("  4. 检查image_urls参数是否工作")
    ...
    if success:
        ...
        print()
        print("关键改进:")
        print("  ✓ 直接HTTP调用Volcano API")
        print("  ✓ 绕过OpenAI SDK限制")
        print("  ✓ 支持官方image_urls参数格式")
        print("  ✓ 支持多模态图片融合(1-10张参考图)")
        print()
        print("技术突破:")
        print("  • 发现OpenAI SDK不支持image_urls参数")
        print("  • 使用requests直接调用Volcano API")
        print("  • 保持完全的API兼容性")
        print()
        print("下一步:")
        print("  1. 在浏览器访问 http://localhost:5003")
        print("  2. 上传参考图片测试图生图")
        ...
        print()
        print("可能原因:")
        print("  1. V6服务器未启动或端口冲突")
        print("  2. Volcano API密钥配置错误")
        print("  3. 网络连接问题")
        print("  4. API参数格式仍需调整")
        print()
        print("建议:")
        print("  - 检查V6服务器日志: v6_debug.log")
        print("  - 访问 http://localhost:5003/logs 查看详细日志")
        print("  - 查看Volcano官方文档确认API格式")
