...
# -*- coding: utf-8 -*-
"""
AI图像生成器 - Web版 V7 (完全修复版)
支持主题输入或参考图片,多种画图风格选择
使用即梦AI(Seedream)模型生成图像

V7修复:
  ✅ 使用正确的zai-sdk导入 (from zai import ZhipuAiClient)
  ✅ 使用正确的image_url格式 (字典格式: {"url": base64_string})
  ✅ 完全修复视觉模型分析错误
  ✅ 修复模型识别显示(SEEDREAM-V6而非GEMINI)
  ✅ 完整的错误处理和日志记录
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('v7_debug.log', encoding='utf-8')
    ]
)
...
def analyze_reference_image(reference_image_path):
    ...
    try:
        ...
        try:
            ...
            client = ZhipuAiClient(api_key=get_config_value('ZHIPU_API_KEY'))

            logging.info("[视觉分析] 使用GLM-4V分析参考图片...")
            ...
            logging.warning("[警告] zai-sdk未安装,使用通用描述")
            return "一张图片"
...
def generate_with_seedream_v7(prompt, reference_image_path, output_path):
    """使用即梦AI(Seedream) V7格式生成图像

    V7关键修复:
    - 完全修复返回值解包问题
    - 正确识别data/image_url响应格式
    - 修复模型显示名称
    - 完整的错误处理
    ...
    try:
        ...
        # V7改进:如果有参考图片,使用image_urls参数(官方格式)
        payload = {
        ...
        if response.status_code == 200:
            ...
            # V7改进: 检查结果键是否存在(兼容不同的响应格式)
            if 'data' in result and len(result['data']) > 0:
                # 标准OpenAI格式
                image_url = result['data'][0].get('url')
                model_used = 'seedream-v7'
...
def generate_with_gemini(prompt, output_path):
    ...
    try:
        # Gemini作为备选方案暂时不可用
        return False, "Gemini备选方案暂未配置", "gemini"
    except Exception as e:
        error_msg = str(e)
        logging.error(f"[错误] Gemini生成失败: {error_msg}")
        return False, f"生成失败: {error_msg}", "gemini"
...
{style_config['prompt_template'].format(theme=reference_image_description)}"""
                ...
                logging.debug(f"  prompt前100字符: {prompt[:100]}...")
        ...
        # V7改进: 使用Seedream V7生成
        logging.info("[步骤2] 开始即梦AI V7生成...")
        success, message, model_used = generate_with_seedream_v7(prompt, reference_image_path, str(output_path))
        ...
        if not success:
            logging.warning(f"[Seedream V7失败] {message}")
            logging.info("[备选] 尝试使用Gemini生成...")
            success, message, model_used = generate_with_gemini(prompt, str(output_path))
...
def view_logs():
    """查看服务器日志"""
    try:
        with open('v7_debug.log', 'r', encoding='utf-8') as f:
            logs = f.read()
...
def main():
    """主函数"""
    print("\n" + "="*80)
    print("                    AI图像生成器 - Web版 V7 (完全修复版)")
    print("="*80)
    print()
    print("启动Web服务器: http://localhost:5007")
    print("请在浏览器中打开上述地址")
    print()
    print("功能特性:")
    print("  ✨ 支持主题描述生成")
    ...
    print()
    print("V7修复:")
    print("  ✅ 使用正确的zai-sdk导入 (from zai import ZhipuAiClient)")
    print("  ✅ 使用正确的image_url格式 (字典格式)")
    print("  ✅ 完全修复视觉模型分析错误")
    print("  ✅ 修复模型识别显示(SEEDREAM-V7而非GEMINI)")
    print("  ✅ 完整的错误处理和日志记录")
    print("="*80)
    print()
    print("💡 调试提示:")
    print("  - 所有print输出都会在浏览器F12控制台中显示")
    print("  - 可访问 http://localhost:5007/logs 查看完整日志")
    print("="*80)
    print()
