...
# -*- coding: utf-8 -*-
"""
AI图像生成器 - Web版 V4 (带控制台日志)
支持主题输入或参考图片,多种画图风格选择
使用即梦AI(Seedream)模型生成图像

V4改进: 添加详细的logging输出,方便调试问题
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('v4_debug.log', encoding='utf-8')  # 同时输出到文件
    ]
)
...
{style_config['prompt_template'].format(theme=reference_image_description)}"""
                ...
                logging.debug(f"{detailed_prompt[:100]}...")
        ...
        if not success:
            logging.warning(f"[Seedream失败] {message}")
...
def view_logs():
    """查看服务器日志"""
    try:
        with open('v4_debug.log', 'r', encoding='utf-8') as f:
            logs = f.read()
...
def main():
    ...
    print()
    print("V4改进:")
    print("  ✅ 添加详细的logging输出,方便调试问题")
    print("  ✅ 支持/file/logs查看实时日志")
    print("="*80)
    print()
    print("💡 调试提示:")
    print("  - 所有print输出都会在浏览器F12控制台中显示")
    print("  - 可访问 http://localhost:5001/logs 查看完整日志")
    print("="*80)
    print()
